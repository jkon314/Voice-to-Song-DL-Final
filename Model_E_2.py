...
class Model(nn.Module):
    def __init__(self):
        ...
        self.decoder = Decoder(lstm_hidden_size=1024,lstm_bidirectional=False)
    ...
    def forward(self,input):
        ...
        target = target[:,:,None] #changed to target tto 
        style_dup = torch.repeat_interleave(target,f_up.shape[1],2) #duplicate style embedding desired number of timesteps
        ...
        encoded_postnet_in = utils.combine_spec_and_style(postnet_out,target.squeeze()) #want to emulate the target flowing through the encoder
